...
router.get("/campgrounds", function(req, res){
    ...
    if(req. query.search){
        ...
        Camp.find({name: regex}).skip((perPage * pageNumber) - perPage).limit(perPage).exec(function(err, searchCamps){
            Camp.count({name: regex}).exec(function (err, count) {
                if(err) {
                    console.log(err);
                    res.redirect("back");
                } else {
                    if (searchCamps.length === 0) {
                        req.flash('error', 'Sorry, no campgrounds match your query. Please try again');
                        return res.redirect('/campgrounds');
                    }
                    res.render("campgrounds/index", {
                        campgrounds: searchCamps,
                        current: pageNumber,
    ...
    //Get all camps from db
        Camp.find({}).skip((perPage * pageNumber) - perPage).limit(perPage).exec(function(err, camps){
